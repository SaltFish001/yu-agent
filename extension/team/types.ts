...
export const TeamSpecSchema = z.object({
  ...
  if (spec.leadAgentId === undefined && spec.members.length > 1) {
    ctx.addIssue({
      ...
      message: 'leadAgentId required when team has multiple members. Add lead: { kind: "subagent_type", subagent_type: "sisyphus" } or mark one member with isLead: true.',
      path: ['leadAgentId'],
    });
  }
}).transform((spec) => {
...
export const RuntimeStateMemberSchema = z.object({
  name: z.string(),
