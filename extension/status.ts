...
import { upsertAgents, upsertCache, upsertLSP, upsertMCP, upsertSummary, upsertTeam } from './db.js'
...
export function writeSnapshot(snapshot: YuStatusSnapshot): void {
  ...
  upsertTeam(
    tag,
    JSON.stringify({ ...(snapshot.team ?? { active: false }), updatedAt: snapshot.updatedAt }),
    snapshot.updatedAt,
  )
  upsertSummary(
    tag,
...
export function writeAgentStatus(agents: AgentStatus[], updatedAt: number = Date.now()): void {
  const tag = getSessionTag()
  upsertAgents(tag, JSON.stringify({ updatedAt, agents }), updatedAt)
}

/** Write MCP status independently (updated by Pi MCP watcher). */
export function writeMCPStatus(servers: MCPServerStatus[], updatedAt: number = Date.now()): void {
  const tag = getSessionTag()
  upsertMCP(tag, JSON.stringify({ updatedAt, servers }), updatedAt)
}

/** Write team mode status independently. */
export function writeTeamStatus(team: TeamStatus, updatedAt: number = Date.now()): void {
  const tag = getSessionTag()
