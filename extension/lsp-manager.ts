...
export class LspManager {
   ...
   */
  async start(name: string, command: string, args: string[], rootPath: string): Promise<void> {
    if (this.process) {
    ...
    // Handle stdout (LSP JSON-RPC messages) via ReadableStream
    ;(async () => {
      try {
        ...
        log.error(`${name} stdout error`, err)
      }
    })()

    // Handle process exit
    proc.exited.then((exitCode) => {
      log.warn(`${name} exited`, { code: exitCode })
      this.process = null
      this.started = false
      ...
      const err = new Error(`LSP server exited (code=${exitCode})`)
    ...
    const _result = (await this.sendRequest('initialize', {
  ...
  private sendMessage(msg: unknown): void {
    ...
    writer.write(encoder.encode(header + body))
    writer.releaseLock()
  }
   ...
   */
  private sendRequest(method: string, params: unknown, timeoutMs = RESPONSE_TIMEOUT_MS): Promise<unknown> {
