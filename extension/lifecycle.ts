...
export class ShutdownManager {
  ...
  registerHandler(_name: string, handler: () => Promise<void>): void {
  ...
  async shutdown(signal: string): Promise<void> {
    ...
    if (this.runningAgents.size > 0) {
      ...
      await Promise.race([this.waitForAgents(30_000), new Promise<void>((r) => setTimeout(r, 30_000))])
    }
  ...
  private waitForAgents(_timeout: number): Promise<void> {
