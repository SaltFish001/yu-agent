...
function cleanupZombieBackgroundRecords(): number {
  try {
    const db = new DatabaseSync(resolve(process.env.HOME || '/home/saltfish', '.yu', 'topics.db'))
...
setInterval(() => {
  ...
  const alive = status.children.filter(
    (c) => c.status !== 'dead' && c.status !== 'stopped' && c.status !== 'spawn_failed',
  )
  if (status.children.length > 0) {
    daemonLog(
      'INFO',
      `Status: ${alive.length} running / ${status.children.length} total (uptime: ${Math.round(status.uptime / 1000)}s)`,
    )
  }
