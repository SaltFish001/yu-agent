...
function actionSpawnChild(topicName: string, promptTemplate: string, eventPayload: Record<string, unknown>): void {
  ...
  try {
    ...
    const topicRow = db.prepare(
      'SELECT id, status FROM topics WHERE LOWER(name) = LOWER(?)'
    ).get(topicName) as { id: string; status: string } | undefined;
...
export function checkAndTriggerOrchestrator(
  ...
  if (depth >= MAX_ORCH_DEPTH) {
    console.warn(`[orchestrator] Circuit breaker triggered: max orchestration depth (${MAX_ORCH_DEPTH}) reached. Breaking chain.`);
    resetOrchDepth();
    return;
  }
