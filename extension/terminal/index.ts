...
import { openSync, readSync, closeSync, existsSync, readdirSync, readlinkSync, statSync } from 'node:fs';
...
const BUF_FLUSH_INTERVAL_MS = 2_000; // flush buffer every 2s for callback
...
function currentUser(): string {
  try {
    return execSync('whoami', { encoding: 'utf-8', timeout: 3_000 }).trim();
  } catch {
...
export function listTerminalProcesses(): TerminalProcess[] {
  ...
  try {
    psOutput = execSync(
      `ps -u "${user}" -o pid=,comm=,lstart= --no-headers 2>/dev/null || true`,
  ...
  for (const line of psOutput.split('\n')) {
    ...
    const isShell = command.includes('bash') ||
    ...
    if (isShell || hasTty) {
      ...
      try {
        const statRaw = execSync(`stat -c '%Y' "${procDir}" 2>/dev/null || echo ${Math.floor(Date.now()/1000)}`, {
          encoding: 'utf-8',
          timeout: 2_000,
        }).trim();
        const ts = parseInt(statRaw, 10);
        if (!isNaN(ts)) startedAt = ts * 1000;
      } catch { /* use fallback */ }
...
function assertOwnProcess(pid: number): void {
  try {
    ...
    const status = execSync(`grep -i '^Uid:' "${statusPath}" 2>/dev/null`, {
      encoding: 'utf-8',
      timeout: 3_000,
    }).trim();
    ...
    const ourUid = parseInt(
      execSync('id -u', { encoding: 'utf-8', timeout: 3_000 }).trim(),
      10,
    );
  ...
  switch (sub) {
    case 'list': {
      ...
      for (const p of procs) {
        const cmd = p.command.length > 30
          ? p.command.slice(0, 27) + '...'
          : p.command.padEnd(30);
    ...
    default:
      return 'Usage: yu terminal list              List terminal processes\n' +
