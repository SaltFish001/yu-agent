...
describe('Logger', () => {
  ...
  it('writes info-level entries to the logs table', async () => {
    ...
    const db = getDb()
    const rows = db.prepare('SELECT * FROM logs WHERE module = ? ORDER BY id').all('test') as Array<
  ...
  it('persists warn level with timestamp', async () => {
    ...
    const db = getDb()
    const rows = db.prepare('SELECT * FROM logs WHERE module = ? AND level = ?').all('test', 'warn') as Array<
      Record<string, unknown>
    >
  ...
  it('writes error entries with serialized error details', async () => {
    ...
    const db = getDb()
    const rows = db.prepare("SELECT * FROM logs WHERE level = 'error' AND module = ?").all('test') as Array<
      Record<string, unknown>
    >
  ...
  it('does NOT persist debug-level entries', async () => {
    ...
    const db = getDb()
    const rows = db.prepare("SELECT * FROM logs WHERE level = 'debug' AND module = ?").all('test') as Array<
      Record<string, unknown>
    >
  ...
  it('persists multiple log entries from different modules', async () => {
    ...
    const db = getDb()
    const rowsA = db.prepare('SELECT * FROM logs WHERE module = ?').all('test-a') as Array<Record<string, unknown>>
    const rowsB = db.prepare('SELECT * FROM logs WHERE module = ?').all('test-b') as Array<Record<string, unknown>>
  ...
  it('persists fatal-level entries', async () => {
    ...
    const db = getDb()
    const rows = db.prepare("SELECT * FROM logs WHERE level = 'fatal' AND module = ?").all('test-logger') as Array<
      Record<string, unknown>
    >
  ...
  it('persists data field alongside log entry', async () => {
    ...
    const db = getDb()
    const rows = db.prepare("SELECT * FROM logs WHERE level = 'info' AND module = ?").all('test-logger') as Array<
      Record<string, unknown>
    >
  ...
  it('includes valid ISO 8601 timestamp in DB entry', async () => {
    ...
    const db = getDb()
    const rows = db.prepare("SELECT * FROM logs WHERE module = ? AND level = 'info'").all('test-logger') as Array<
      Record<string, unknown>
    >
