...
import { trackAgent, resetTracker } from '../../extension/tracker.js';
...
describe('Agent tracking', () => {
  ...
  it('writes running status to agent_runs', () => {
    ...
    const db = getDb();
    const row = db
      .prepare('SELECT * FROM agent_runs WHERE agent_id = ?')
      .get('test-1') as Record<string, unknown> | undefined;
  ...
  it('transitions from running to completed', () => {
    ...
    const row = db
      .prepare('SELECT * FROM agent_runs WHERE agent_id = ?')
      .get('test-2') as Record<string, unknown> | undefined;
  ...
  it('records failed status with error message', () => {
    ...
    const row = db
      .prepare('SELECT * FROM agent_runs WHERE agent_id = ?')
      .get('test-3') as Record<string, unknown> | undefined;
