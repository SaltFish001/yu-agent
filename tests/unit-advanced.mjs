...
// ── 8. template.ts — validateOutput for all 7 types ──
console.log('\n🔧 template — validateOutput for all types');

// search
await testAsync('validateOutput: search with valid results', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('search', { results: [] });
  strictEqual(result.valid, true);
  strictEqual(result.errors.length, 0);
});

await testAsync('validateOutput: search with results items', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const data = { results: [{ source: 'codebase', path: 'src/a.ts', snippet: 'hello' }] };
  const result = mod.validateOutput('search', data);
  strictEqual(result.valid, true);
});

await testAsync('validateOutput: search missing results array (invalid)', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('search', {});
  strictEqual(result.valid, false);
  ok(result.errors[0].includes('results'));
});

await testAsync('validateOutput: search with null results (invalid)', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('search', { results: null });
  strictEqual(result.valid, false);
});

// commit
await testAsync('validateOutput: commit with committed status', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('commit', { status: 'committed', hash: 'abc123' });
  strictEqual(result.valid, true);
});

await testAsync('validateOutput: commit with nothing_to_commit', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('commit', { status: 'nothing_to_commit' });
  strictEqual(result.valid, true);
});

await testAsync('validateOutput: commit with invalid status', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('commit', { status: 'invalid' });
  strictEqual(result.valid, false);
  ok(result.errors[0].includes('status'));
});

await testAsync('validateOutput: commit with missing status', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('commit', {});
  strictEqual(result.valid, false);
});

// plan
await testAsync('validateOutput: plan with valid complete status and modules', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('plan', { status: 'complete', modules: [] });
  strictEqual(result.valid, true);
});

await testAsync('validateOutput: plan with invalid status', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('plan', { status: 'incomplete', modules: [] });
  strictEqual(result.valid, false);
  ok(result.errors[0].includes('status'));
});

await testAsync('validateOutput: plan with missing modules', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('plan', { status: 'complete' });
  strictEqual(result.valid, false);
  ok(result.errors[0].includes('modules'));
});

await testAsync('validateOutput: plan with null modules', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('plan', { status: 'complete', modules: null });
  strictEqual(result.valid, false);
});

await testAsync('validateOutput: plan with valid modules data', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const data = { status: 'complete', summary: 'plan', modules: [{ name: 'm1', files: ['a.ts'], independent: true }] };
  const result = mod.validateOutput('plan', data);
  strictEqual(result.valid, true);
});

// doc
await testAsync('validateOutput: doc with valid success status', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('doc', { status: 'success', files_written: ['doc.md'] });
  strictEqual(result.valid, true);
});

await testAsync('validateOutput: doc with invalid status', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('doc', { status: 'failed' });
  strictEqual(result.valid, false);
  ok(result.errors[0].includes('status'));
});

await testAsync('validateOutput: doc with missing status', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('doc', {});
  strictEqual(result.valid, false);
});

// Non-object output
await testAsync('validateOutput: non-object output (null)', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('coding', null);
  strictEqual(result.valid, false);
  ok(result.errors[0].includes('not an object'));
});

await testAsync('validateOutput: non-object output (string)', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('coding', 'hello');
  strictEqual(result.valid, false);
});

await testAsync('validateOutput: non-object output (number)', async () => {
  const mod = await import(resolve(ROOT, 'dist/extension/template.js'));
  const result = mod.validateOutput('coding', 42);
  strictEqual(result.valid, false);
});
